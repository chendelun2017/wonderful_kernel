...
# 需要注意，最后需要将预测出的平滑数据还原，而还原过程就是log1p的逆运算expm1:exp(x) - 1。
...
#判断出features矩阵中列为对象的列，将列名存入objects数组。对于features矩阵中的各个列对象，将其列中的空值填充为'None'
objects = []
...
#%%
######################特征删除和融合创建新特征-【开始】###################
#删除一些特征。df.drop（‘列名’, axis=1）代表将‘列名’中对应的列标签（们）一列一列依次删掉。
...
submission.to_csv("Chinese_tutorial_v1.csv", index=False)
...
sub_1 = pd.read_csv('./input/Submit/House_Prices_submit.csv')
sub_2 = pd.read_csv('./input/Submit/hybrid_solution.csv')
sub_3 = pd.read_csv('./input/Submit/lasso_sol22_Median.csv')
...
#%%
#以csv文件的形式输出预测值
submission.to_csv("Chinese_tutorial_v2.csv", index=False)
